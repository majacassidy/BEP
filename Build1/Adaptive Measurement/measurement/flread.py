...
class FileReader:
    ...
    def readColumns(self, columns = "ALL"):
        '''Opens the file and reads the data from it into a numpy array called numData'''
    ...
    def makeMesh(self,xcol=1,ycol=0,zcol=6):
        ...
        j = 0;
        for i in arange(y.size,z.size,y.size):
            self.Z[:,j] = z[i-y.size:i];
            j = j+1;
